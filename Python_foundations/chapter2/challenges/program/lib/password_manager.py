...
class PasswordManager:
    def __init__(self):
        self.passwords = {}
    def add(self,servName:str, passw:str):
        specials = '!@$%&'
        if len(passw) > 7:
            for char in specials:
                if char in passw:
                    self.passwords[servName] = passw


    def get_for_service(self,servName:str):
        if servName in self.passwords:
            return self.passwords[servName]
        
    def list_services(self):
        return [services for services in self.passwords]
# Methods:
#   1. Name: __init__
#      Arguments: none
#   2. Name: add
